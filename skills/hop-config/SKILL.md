...
| `@hop-window-rename` | `off` | `on` \| `off` | Auto-rename the tmux window of each Claude pane to `<state-icon> <task summary>` (project name until a task title exists), updated on every state change. State icons come from `@hop-status-format` tokens (fallback: built-in icons). tmux `automatic-rename` is restored when the session ends. |
